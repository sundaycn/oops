...
@interface MCDirectoryWatcher (MCDirectoryWatcherPrivate)
- (BOOL)startMonitoringDirectory:(NSString *)dirPath;
- (void)kqueueFired;
@end

@implementation MCDirectoryWatcher
@synthesize delegate;
...
+ (MCDirectoryWatcher *)watchFolderWithPath:(NSString *)watchPath delegate:(id)watchDelegate
{
	MCDirectoryWatcher *retVal = NULL;
	if ((watchDelegate != NULL) && (watchPath != NULL))
	{
		MCDirectoryWatcher *tempManager = [[MCDirectoryWatcher alloc] init];
...
@implementation MCDirectoryWatcher (MCDirectoryWatcherPrivate)
...
{
    MCDirectoryWatcher *obj;
	
    obj = (__bridge MCDirectoryWatcher *)info;
    assert([obj isKindOfClass:[MCDirectoryWatcher class]]);
