var app = {
	...
	loadData: function (url, data, success, fail, args) {
		...
		if (typeof args == "object") {
			for (key in args) {
				if (key == "showMask" && args[key] && isAPP) {
					isShowMask = true;
				}
			}
		}
		if (isShowMask) {
			window.APP.showMask("正在搜索...");
		}
	...
	formatDate: function (timestamp, formatString) {
		if (timestamp) {
			var fstring = "yyyy-MM-dd";
			if (formatString) {
				fstring = formatString;
			}
			return new Date(timestamp).format(fstring);
		} else {
			return "";
		}
	},
	...
	// webview委托
	delegation: function (method, args, callback) {
		var browser = this.getBrowser();
		if (browser.iPhone && window.APP) {
			return eval("window.APP."+ method + "("+ (args || "")+")");
		} else {
...
function setTreeList (rs) {
	...
	$(rs).each(function () {
		if (this.nocheck != "true") {
			if (this.isParent == "true") {
				...
				html += '<li class="edit leaf" data-expand="'+ this.expand +'" data-customInfo="'+ this.customInfo +'" data-id="'+ this.id +'" data-name="'+ this.name +'">'+
							'<a href="javascript:void(0)" class="t">'+
								'<h3>'+ this.name +'</h3>'+
								'<p>'+ data['pName'] +'</p>'+
							'</a>'+
	...
	$(".eoa-data-list .leaf").unbind().click(function () {
		var $cb = $(".check", this),
			...
			userCustomInfo = userId.split("_")[1],
			checkedid = userId.split("_")[1];
...
function setTreeMailList (rs) {
	...
	$(rs).each(function () {
		if (this.nocheck != "true") {
			if (this.isParent == "true") {
				html += '<li class="edit folder">'+
							'<a href="maillist.html?cleartop=true&from='+ data.from +'&selectboxid='+ data.selectboxid +'&treeId='+ this.id +'&pName='+ escape(this.name || "") +'" class="t">'+
								'<h3>'+ this.name +'</h3>'+
							'</a>'+
							'<span class="ico">></span>'+
						'</li>'
	...
	if (rs.success) {
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				html += '<li >'+
							'<a href="viewclient.html?id='+ this.id +'" class="t nocheck"> '+
								'<h3>'+ (this.customerName || "") +'</h3>'+
								'<P>'+ (this.customerTypeName || "") +"，"+ (this.areaName || "") + (this.address || "") +'</p>'+
							'</a>'+
							'<span class="ico">></span>'+
						'</li>';
			});
...
// 选择客户
function setSelectClient (rs, args) {
	if (rs.success) {
		var html = "";
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				html += '<li data-id="'+ this.id +'" >'+
							'<a href="javascript:void(0)" class="t nocheck"> '+
								'<h3>'+ (this.customerName || "") +'</h3>'+
								'<P>'+ (this.customerTypeName || "") +"，"+ (this.areaName || "") + (this.address || "") +'</p>'+
							'</a>'+
							'<span class="ico">></span>'+
		...
		$(".eoa-data-list > ul").append(html);
		$(".eoa-data-list > ul li").click(function () {
			var customerId = $(this).data("id"),
				redirectPage = "addbusiness.html";
			if (args && args.redirect) {
				redirectPage = args.redirect;
			}
			window.location.href = redirectPage + "?customerId="+ customerId;
			window.APP.finish(false);
			return false;
		});
...
// 客户详情
function setClientView (rs) {
	if (rs.success) {
		if (rs.message.tcrmCustomerBasicInfo) {
			if (rs.message.tcrmCustomerBasicInfo.customerName) {
				var letter = rs.message.tcrmCustomerBasicInfo.customerName.trim().substring(0, 1);
				$(".view-client .hd .icon").html(letter);
				$(".view-client .hd .name").html(rs.message.tcrmCustomerBasicInfo.customerName || "");
			}
			$(".view-client .hd .address").html((rs.message.tcrmCustomerBasicInfo.areaName || "") 
				+ (rs.message.tcrmCustomerBasicInfo.address || ""));
				
			$("#clientType .txt").html((rs.message.tcrmCustomerBasicInfo.customerTypeName || ""));
			if(rs.message.tcrmCustomerBasicInfo.phone) {
				$("#clientTel").show();
				$("#clientTel .txt").html((rs.message.tcrmCustomerBasicInfo.phone || ""));
				$("#clientTel .icon a").attr("href", "tel:"+ rs.message.tcrmCustomerBasicInfo.phone);
			} else {
				$("#clientTel").hide();
			}
			$("#clientRemark .txt").html((rs.message.tcrmCustomerBasicInfo.remark || "无"))
			
		}
		if (rs.message.tcrmCustomerContactList) {
			var html = '';
			$(rs.message.tcrmCustomerContactList).each(function () {
				if (this.ismain == "Y") {
					$("#mainContact, #mainContactDetail").show();
					$("#clientNamesex .txt").html((this.name || "") + " / " + (this.sex == "F" ? "女" : "男"));
					$("#clientPosition .txt").html((this.department || "") + " / " + (this.position || ""));
					if (this.mobilePhone) {
						$("#clientTelphone .txt").html(this.mobilePhone);
						$("#clientTelphone .icon a.msg").attr("href", "smsto:"+ this.mobilePhone);
						$("#clientTelphone .icon a.tel").attr("href", "tel:"+ this.mobilePhone);
					}
					if (this.phone) {
						$("#clientOfficePhone").show();
						$("#clientOfficePhone .txt").html(this.phone);
						$("#clientOfficePhone .icon a.tel").attr("href", "tel:"+ this.phone);
					}
					if (this.officeRoom) {
						$("#clientRoom").show();
						$("#clientRoom .txt").html(this.officeRoom);
					}
					if (this.email) {
						$("#clientEmail").show();
						$("#clientEmail .txt").html(this.email);
						$("#clientEmail .icon a.mail").attr("href", "mailto:"+ this.email);
					}
					if (this.remark && this.remark.indexOf("telId") <= 0) {
						$("#mainContactRemark").show();
						$("#mainContactRemark .txt").html(this.remark || "");
					}
					if (this.live) {
						$("#clentLike").show();
						$("#clentLike .txt").html(this.live);
					}
					if (this.birthday) {
						$("#clentBirth").show();
						$("#clentBirth .txt").html(app.formatDate(this.birthday, "yyyy年MM月dd日"));
					}
					
				} else {
					$("#otherContacts").show();
					html += '<ul class="view-panel">'+
									'<li class="single-line">'+
										'<p class="label">姓名/性别：</p>'+
										'<p class="txt">'+ (this.name || "") + " / " + (this.sex == "F" ? "女" : "男") +'</p>'+
									'</li>'+
									'<li class="single-line">'+
										'<p class="label" >部门/职位：</p>'+
										'<p class="txt">'+ (this.department || "") + " / " + (this.position || "") +'</p>'+
									'</li>'+
									'<li class="single-line">'+
										'<p class="label" >办公房间：</p>'+
										'<p class="txt">'+ this.officeRoom +'</p>'+
									'</li>'+
									'<li class="single-line">'+
										'<p class="label">移动电话：</p>'+
										'<p class="txt">'+ (this.mobilePhone || "") +'</p>'+
										'<p class="icon"><a href="smsto:'+ (this.mobilePhone || "") +'" class="msg"></a></p>'+
										'<p class="icon"><a href="tel:'+ (this.mobilePhone || "") +'" class="tel"></a></p>'+
									'</li>'+
									'<li class="single-line">'+
										'<p class="label">办公电话：</p>'+
										'<p class="txt">'+ (this.phone || "") +'</p>'+
										'<p class="icon"><a href="tel:'+ (this.phone || "") +'" class="tel"></a></p>'+
									'</li>'+
									'<li class="single-line">'+
										'<p class="label">电子邮件：</p>'+
										'<p class="txt">'+ (this.email || "") +'</p>'+
										'<p class="icon"><a href="mailto:'+ (this.email || "") +'" class="mail"></a></p>'+
									'</li>'+
									'<li class="multi-line">'+
										'<p class="label">客户生日：</p>'+
										'<p class="txt">'+ (this.birthday == "undefined" ? "" : app.formatDate(this.birthday, "yyyy年MM月dd日")) +'</p>'+
									'</li>'+
									'<li class="multi-line">'+
										'<p class="label">客户爱好：</p>'+
										'<p class="txt">'+ (this.live || "") +'</p>'+
									'</li>'+
									'<li class="multi-line">'+
										'<p class="label">相关备注：</p>'+
										'<p class="txt">'+ (this.remark || "") +'</p>'+
									'</li>'+
								'</ul>';
				}
			});
			
			$(".view-client").append(html);
...
function setMyVisit (rs) {
	if (rs.success) {
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				var bubbleClass = "",
					bubbleTxt = (this.num || 0) + "次";
				
				html += '<li >'+
							'<a href="visitlist.html?filter_LIKES_customerId='+ this.id +'" class="t nocheck"> '+
								'<h3>'+ (this.customerName || "") +'</h3>'+
								'<P>上次拜访登记：'+ this.lastTime +'</p>'+
							'</a>'+
							'<span class="bubble r40">'+ bubbleTxt +'</span>'+
							'<span class="ico">></span>'+
						'</li>';
			});
...
function setTimeline () {
	...
	$("#pageindex").height($(".eoa-timeline-content").height());
	$(".eoa-timeline-content img").load(function () {
		$(this).width(W - 30).show();
	});
}

// 我的商机
function setMyBusiness (rs) {
	if (rs.success) {
		var html = "";
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				html += '<li >'+
							'<a href="viewbusiness.html?id='+ this.id +'" class="t nocheck"> '+
								'<h3>'+ (this.customerId || "") +'</h3>'+
								'<P>'+ (this.businessDescribe || "") +'</p>'+
							'</a>'+
							'<span class="ico">></span>'+
						'</li>';
			});
...
// 商机详情
function setBusinessView (rs) {
	if (rs.success) {
		$("#clientName .txt").html(rs.message.customerId || "");
		$("#busfrom .txt").html(rs.message.businessSource || "");
		$("#busmoney .txt").html(rs.message.expectMoney || "");
		$("#buscontact .txt").html(rs.message.mainPerson || "");
		$("#bustype .txt").html(rs.message.businessType || "");
		$("#busstatus .txt").html(rs.message.businessPhase || "");
		$("#busother .txt").html(rs.message.otherInfo || "");
		$("#busdescr .txt").html(rs.message.businessDescribe || "");
		$("#busremark .txt").html(rs.message.businessRemark || "");
	}
}

// 竞争对手
function setCompetitorView (rs) {
	if (rs.success) {
		var html = "";
			
		$(rs.message.result).each(function (i) {
			var phone = this.contactInfo || "";
			
			html += '<p class="sub-title"><i>竞争对手</i></p>'+
					'<ul class="view-panel">'+
						'<li class="single-line">'+
							'<p class="label">名称：</p>'+
							'<p class="txt">'+ (this.name || "") +'</p>'+
						'</li>'+
						'<li class="single-line">'+
							'<p class="label" >类型：</p>'+
							'<p class="txt">'+ (this.competitionType || "") +'</p>'+
						'</li>'+
						'<li class="multi-line">'+
							'<p class="label" >优势分析：</p>'+
							'<p class="txt">'+ (this.advantageAnalysis || "") +'</p>'+
						'</li>'+
						'<li class="multi-line">'+
							'<p class="label">劣势分析：</p>'+
							'<p class="txt">'+ (this.disadvantages || "") +'</p>'+
						'</li>'+
						'<li class="multi-line">'+
							'<p class="label">应对方法：</p>'+
							'<p class="txt">'+ (this.response || "") +'</p>'+
						'</li>'+
						'<li class="single-line" style="display:'+ (phone == "" ? "none" : "") +'">'+
							'<p class="label">联系信息：</p>'+
							'<p class="txt">'+ (this.contactInfo || "") +'</p>'+
							'<p class="icon"><a href="tel:'+ (this.contactInfo || "") +'" class="tel"></a></p>'+
						'</li>'+
						'<li class="multi-line">'+
							'<p class="label">相关备注：</p>'+
							'<p class="txt">'+ (this.remark || "") +'</p>'+
...
// 拜访历史记录
function setVisitList (rs, info) {
	var getCustomerContact = function (customerContactId) {
		var customerContact = {};
		$(info.message.tcrmCustomerContactList).each(function () {
			if (this.id == customerContactId) {
				customerContact = this;
			}
		});
	...
	if (rs.success) {
		...
		$(rs.message.result).each(function (i) {
			var contact = getCustomerContact(this.customerContactId),
				isTel = false,
				isImg = this.isTakingPictures != "undefined" && this.isTakingPictures == "Y" ? true : false,
				isBussnessFollow = this.isBusiness == "Y";
			...
			if (isTel) {
				...
				var imgDiv = "", followDiv = "", addressDiv = "";
				if (isImg) {
					imgDiv = '<div class="img" >'+
								'<a href="javascript:void(0)"><img src="../images/IMG_8020.jpg" width="620"/></a>'+
							'</div>';
				}
				
				if (isBussnessFollow) {
					followDiv = '<div class="follow">'+
									'<p>跟进：'+ (this.businessName || "") +'</p>'+
									'<p>阶段：'+ (this.businessPhaseName || "") +'</p>'+
									'<p>备注：'+ (this.businessRemark || "") +'</p>'+
								'</div>'
				}
				
				if (this.callAddress) {
					addressDiv = '<div class="follow">'+
									'<p>定位：'+ (this.callAddress || "") +'</p>'+
								'</div>'
				}
				...
				html += '<li class="edt">'+
							'<div class="desc">'+
								'<div class="txt">'+
									'<h3>'+ (this.callTypeName || "") +"(联系人："+ contact.name + ')</h3>'+
									'<p>'+ (this.callConclusion || "") +'</p>'+
								'</div>'+
								'<span class="date">'+ app.getTimeDuration(this.createDate) +'</span>'+
							'</div>'+
							followDiv +
							imgDiv +
							addressDiv + 
		...
		$(".eoa-timeline-content .bd").append(html);
...
// 备案公共查询
function setScanView (rs) {
	if (rs.success) {
		if (rs.message) {
			if (rs.message.name) {
				var letter = rs.message.name.trim().substring(0, 1);
				$(".view-client .hd .icon").html(letter);
				$(".view-client .hd .name").html(rs.message.name|| "");
			}
			$(".view-client .hd .address").html(rs.message.businessAddress || "");
			$("#recordCode .txt").html((rs.message.recordCode || ""));
			$("#license .txt").html((rs.message.license || ""));
			$("#businessScope .txt").html((rs.message.businessScope || ""));
			$("#leaderName .txt").html((rs.message.leaderName || ""));
			if (rs.message.leaderPhone ) {
				$("#leaderPhone").show();
				$("#leaderPhone .txt").html((rs.message.leaderPhone || ""));
				$("#leaderPhone .icon a.msg").attr("href", "smsto:"+ rs.message.leaderPhone);
				$("#leaderPhone .icon a.tel").attr("href", "tel:"+ rs.message.leaderPhone);
			}
			if (rs.message.phone ) {
				$("#phone").show();
				$("#phone .txt").html((rs.message.phone || ""));
				$("#phone .icon a.tel").attr("href", "tel:"+ rs.message.phone);
			}
			$("#address .txt").html((rs.message.address || ""));
			$("#postCode .txt").html((rs.message.postCode || ""));
			$("#allPostmanCount .txt").html((rs.message.allPostmanCount || "0") + "人");
			$("#employeeCount .txt").html((rs.message.employeeCount || "0") + "人");
			$("#postmanCount .txt").html((rs.message.postmanCount || "0") + "人");
			$("#postmanRate .txt").html((rs.message.postmanRate || "") + "%");
			$("#map a").attr("href", "map.html?address="+ escape(rs.message.address|| "") +"&name="+ escape(rs.message.name|| "") +"&mapLocalX="+ (rs.message.mapLocalX || "")+ "&mapLocalY="+ (rs.message.mapLocalY || ""));
	...
	if (rs.success) {
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				html += '<li>'+
							'<a href="branch.html?recordId='+ this.id +'" class="t nocheck" style="padding-left:10px;"> '+
								'<h3>'+ (this.name || "") +'</h3>'+
								'<P>'+ (this.branchTypeString || "") +"，负责人："+ (this.leaderName || "") +'</p>'+
							'</a>'+
							//'<span class="bubble '+ bubbleClass +'">'+ (this.statusString || "") +'</span>'+
							'<span class="ico">></span>'+
						'</li>';
			});
	...
	if (rs.success) {
		if (rs.message) {
			if (rs.message.name) {
				...
				$(".view-client .hd .icon").html(letter);
				$(".view-client .hd .name").html(rs.message.name|| "");
				$("title").html(rs.message.name);
			}
			$(".view-client .hd .address").html(rs.message.businessAddress || "");
			$("#recordCode .txt").html((rs.message.recordCode || ""));
			$("#license .txt").html((rs.message.license || ""));
			$("#businessScope .txt").html((rs.message.businessScope || ""));
			$("#businessAddress .txt").html(rs.message.businessAddress || "");
			$("#leaderName .txt").html((rs.message.leaderName || ""));
			if (rs.message.leaderName) {
				$("#leaderName .more").parent(".icon").show();
				$("#leaderName .more").attr("href", "branchleader.html?recordId="+ rs.message.id)
			}
			if (rs.message.leaderPhone ) {
				$("#leaderPhone").show();
				$("#leaderPhone .txt").html((rs.message.leaderPhone || ""));
			...
			$("#postCode .txt").html((rs.message.postCode || ""));
			$("#allPostmanCount .txt").html((rs.message.allPostmanCount || "0") + "人");
			$("#allPostmanCount .txt").html((rs.message.employeeCount || "0") + "人");
			if (rs.message.employeeCount && rs.message.employeeCount > 0) {
				$("#allPostmanCount .icon").show();
				$("#allPostmanCount .icon a.more").attr("href", "personlist.html?recordId="+ rs.message.id + "&type=employees");
			}
			$("#postmanCount .txt").html((rs.message.postmanCount || "0") + "人");
			if (rs.message.postmanCount && rs.message.postmanCount > 0) {
				$("#postmanCount .icon").show();
				$("#postmanCount .icon a.more").attr("href", "personlist.html?recordId="+ rs.message.id + "&type=postman");
			}
			$("#postmanRate .txt").html((rs.message.postmanRate || "") + "%");
			$("#map a").attr("href", "map.html?address="+ escape(rs.message.address|| "") +"&name="+ escape(rs.message.name|| "") +"&mapLocalX="+ (rs.message.mapLocalX || "")+ "&mapLocalY="+ (rs.message.mapLocalY || ""));
			
			$("#branchCode .txt").html((rs.message.branchCode || ""));
			$("#branchType .txt").html((rs.message.branchTypeString || ""));
			$("#leaderCertifyType .txt").html((rs.message.leaderCertifyTypeString || ""));
			$("#leaderCertifyCode .txt").html((rs.message.leaderCertifyCode || ""));
			$("#status .txt").html((rs.message.statusString || ""));
			$("#corPerson .txt").html((rs.message.corPersonString || ""));
			$("#corDateString .txt").html((rs.message.corDateString || ""));
			$("#corRemark .txt").html((rs.message.corRemark || ""));
			$("#acceptPerson .txt").html((rs.message.acceptPersonString || ""));
			$("#acceptDateString .txt").html((rs.message.acceptDateString || ""));
			$("#acceptRemark .txt").html((rs.message.acceptRemark || ""));
			$("#auditPerson .txt").html((rs.message.auditPersonString || ""));
			$("#auditDateString .txt").html((rs.message.auditDateString || ""));
			$("#auditRemark .txt").html((rs.message.auditRemark || ""));
...
function setBranchLeaderView (rs) {
	console.log(rs)
	if (rs.success) {
		if (rs.message) {
			$("title").html(rs.message.name || "负责人");
			$("#name .txt").html((rs.message.name || ""));
			$("#sex .txt").html((rs.message.sex || ""));
			$("#idCard .txt").html((rs.message.idCard || ""));
			$("#houseRegister .txt").html(rs.message.houseRegister || "");
			$("#address .txt").html((rs.message.address || ""));
			if (rs.message.phone ) {
				$("#phone").show();
				$("#phone .txt").html((rs.message.phone || ""));
				$("#phone .icon a.msg").attr("href", "smsto:"+ rs.message.phone);
				$("#phone .icon a.tel").attr("href", "tel:"+ rs.message.phone);
			}
			
			$("#personExp .txt").html((rs.message.personExp || ""));
			$("#workExp .txt").html((rs.message.workExp || ""));
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				if (data.type == "employees") {
					html += '<li>'+
								'<a href="javascript:void(0);" class="t nocheck" style="padding-left:10px;"> '+
									'<h3>'+ (this.name || "") +'('+ (this.stationString || "未设置岗位") +')</h3>'+
									'<P>身份证:'+ (this.idCard || "") +'</p>'+
								'</a>'+
								//'<span class="bubble '+ bubbleClass +'">'+ (this.statusString || "") +'</span>'+
								//'<span class="ico">></span>'+
...
// 分支机构查询列表
function setBranchSearchList (rs) {
	console.log(rs)
	if (rs.success) {
		var html = "";
		...
		if (rs.message.totalCount > 0) {
			$(rs.message.result).each(function () {
				...
				html += '<li>'+
							'<a href="branchsearchmap.html?filter_EQS_id='+ this.id +'&name='+ escape(this.name) +'" class="t nocheck" style="padding-left:10px;"> '+
								'<h3>'+ (this.name || "") +'</h3>'+
								'<P>地址：'+ (this.address || "未填写。") +'</p>'+
							'</a>'+
							//'<span class="bubble '+ bubbleClass +'">'+ (this.statusString || "") +'</span>'+
							'<span class="ico">></span>'+
						'</li>';
